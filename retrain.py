...
def main(_):
  ...
  for i in range(FLAGS.how_many_training_steps):
    ...
    if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
      train_accuracy, cross_entropy_value = sess.run(
          ...
          feed_dict={bottleneck_input: train_bottlenecks,
                     ground_truth_input: train_ground_truth})
      #print('Step %d: Train accuracy = %.1f%%' % (i,
      #                                                train_accuracy * 100))
      from time import gmtime, strftime
      currtime = strftime("[%H:%M:%S]", gmtime())
      print('%s\tStep #: %d of %d\tLoss = %f' % (currtime, i, FLAGS.how_many_training_steps,
                                                 cross_entropy_value))
      step_list.append(i)
  ...
  import csv
  with open("performance/inception_loss.csv", 'w', newline='') as csvfile:
    fieldnames = ['Step', 'Loss']
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
    writer.writeheader()
    for i in range(len(step_list)):
      writer.writerow({'Step': step_list[i], 'Loss': loss_list[i]})
  print('Total train time = %0.4f' % train_time)
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--image_dir',
      type=str,
      default='dataset/jpeg',
      help='Path to folders of labeled images.'
  )
  parser.add_argument(
      '--output_graph',
      type=str,
      default='models/inception_graph.pb',
      help='Where to save the trained graph.'
  )
  parser.add_argument(
      '--output_labels',
      type=str,
      default='models/inception_labels.txt',
      help='Where to save the trained graph\'s labels.'
  )
  parser.add_argument(
      '--summaries_dir',
      type=str,
      default='models/inception_retrain_logs',
